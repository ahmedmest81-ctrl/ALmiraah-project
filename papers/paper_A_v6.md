...
- A measurement of wazn geometry in CAMeLBERT-ca at the embedding level: across 951 word forms spanning 200 reviewed triliteral root families, morphological pattern membership is significantly clustered and the four broad-coverage derived patterns are linearly decodable by analogy completion (mean cosine 0.924–0.983; n = 98–200), establishing that templatic morphology surfaces as separable geometric structure rather than as a mere co-occurrence correlate. We additionally report that the per-root pattern offsets are not a single consistent translation vector, bounding the strength of the “geometric operator” interpretation.
...
We assembled a lexicon of 951 word forms spanning 200 reviewed triliteral root families (after a manual review pass that removed 122 mis-segmented or mis-annotated forms and corrected 26 others from a 1,073-form raw set). The reviewed run contains 200 unique triliteral family labels; the available log documents generation plus manual filtering, but not a separate corpus-attestation audit, so we do not describe every family as independently “well-attested.” For each root we recorded the surface form for each retained morphological pattern. Five patterns have broad coverage across the family set — Form I (the base, V1), Form II (faʿʿala), the verbal noun (maṣdar), the active participle (fāʿil), and the passive participle (mafʿūl), each with n ≥ 98 — and we restrict per-pattern claims to these; the remaining patterns (Forms IV–X, adjectival, broken plural, and mixed forms) are individually sparse (n < 16) and reported only descriptively. We computed three measures. (i) *Analogy completion*: for each pattern p we formed the cross-root mean offset d(p) = mean over roots of (vec(derived) − vec(base)), and for each held-out root r predicted vec(r, p) = vec(r, base) + d(p), scoring cosine similarity against the actual contextual embedding. (ii) *Pattern clustering*: for each pattern we compared mean within-pattern to mean cross-pattern cosine similarity (gap test, permutation p-value). (iii) *Offset consistency*: the mean pairwise cosine similarity among the per-root offset vectors of a pattern, measuring whether the transformation is a single shared direction. All embeddings are mean-pooled contextual vectors from CAMeLBERT-ca over three carrier sentences per word.
...
The primary test was a partial Spearman correlation between Abjad distance and mean attention weight, controlling for positional distance and corpus co-occurrence frequency. In Arabic, cross-root pairs (n = 2,322,977) were the principal sample. In Hebrew, where the heuristic root extractor confidently resolves only a minority of pairs (§5.4), the principal sample comprised all 12,081,207 analyzable pairs: 1,468,657 confidently classified different-root, 58,776 classified same-root, and 10,553,774 unresolved. The confidently same-root subsets (n = 80,647 Arabic; n = 58,776 Hebrew) were also analysed separately as pipeline-sensitivity controls; the Hebrew same-root subset remains included in the primary all-pairs result.
...
**Analogy completion.** For each of the four broad-coverage derived patterns, a leave-one-out prediction vec(r, base) + d(p) — where d(p) is the cross-root mean offset for pattern p — recovers the held-out derived form at mean cosine similarity above 0.923 (Table 1; n = 98–200). The result is strongest precisely where the data are densest: Form II (n = 195) and the verbal noun (n = 200) are the two highest-scoring patterns. Broken plural is included in Table 1 as a sparse descriptive comparison (n = 9), not as part of the broad-coverage claim.
...
**Table 1.** Leave-one-out analogy completion for the four broad-coverage derived patterns in CAMeLBERT-ca, with broken plural included as a sparse descriptive comparison (951 word forms spanning 200 reviewed root families). Predictions take the form vec(r, base) + d(p), where d(p) is the cross-root mean offset for pattern p. Remaining patterns in the lexicon (Forms IV, V, VI, VIII, X, adjectival, and mixed) are individually sparse (n < 12) and are reported in the supplementary material; all score ≥ 0.90, but we make no per-pattern claim at those cell sizes.
...
**Pattern clustering.** Words sharing a morphological pattern are more mutually similar than words of differing patterns. The within-pattern minus cross-pattern similarity gap is positive and statistically significant for every broad-coverage pattern (V1: gap = 0.017, p = 2×10⁻¹⁴; PP: gap = 0.024, p = 1×10⁻⁴⁰; VN: gap = 0.015, p = 3×10⁻¹¹; V2: gap = 0.009, p = 2×10⁻⁵; AP: gap = 0.007, p = 4×10⁻⁴; n = 100–209 word forms per pattern). The upper value is not a typo: the verbal-noun clustering test contains 209 retained forms across 200 root families because nine families each contribute two distinct verbal-noun forms. The gaps are small in absolute terms because all similarities sit in a compressed 0.82–0.87 band — a known anisotropy property of contextual encoders (Ethayarajh, 2019; Gao et al., 2019) — but they are highly significant: pattern membership is reliably separable even within the anisotropic regime.
...
The Hebrew control returned partial ρ = −0.0779 (n = 12,081,207 all analyzable pairs, p beyond float64 precision). The exploratory confidently different-root subset returned ρ = −0.0284 (n = 1,468,657), while the same-root subset returned ρ = −0.334 (n = 58,776), confirming that AlephBERT detects same-root coupling and is, if anything, more sensitive to root-level coupling than CAMeLBERT-ca. The binned analysis of the primary all-pairs sample showed monotonic decrease in mean attention from the most-proximate bin (Δ 0–10: 0.0167) to the most-distal (Δ > 200: 0.0125), with the same gradient structure as the Arabic result.
...
**Sample-definition note on the decision rule.** The pre-registration specified the Arabic pipeline applied mutatis mutandis. In Arabic the principal sample is the cross-root pair set; in Hebrew, restricting the principal analysis to confidently different-root pairs would discard 87.4% of observations because of the heuristic extractor’s limited coverage, so the Hebrew principal sample is the all-analyzable-pairs set and the decision rule is evaluated on it. We report the like-for-like comparison transparently: on the confidently different-root subset, ρ = −0.0284 (n = 1,468,657) — a value that would fall within the pre-registered Outcome C band [−0.01, −0.03] had that subset been the principal sample, at approximately half the magnitude of the Arabic cross-root primary. The divergence between the all-pairs and subset estimates is expected in direction: explicit same-root pairs (ρ = −0.334) and the unresolved majority, which contains an unknown fraction of undetected same-root pairs, both enrich the all-pairs sample with strong morphological coupling. The substantive conclusion is unaffected by which reading is preferred — under Outcome A the signal is morphologically general; under Outcome C it is present but attenuated, with the register confound unresolved — and neither reading supports the Abjad-unique interpretation that the control was designed to test. The gematria permutation null reported below is valid for the all-pairs sample on which it was run, but it does not replace a separate permutation null on the confidently different-root subset.
...
The four-level pattern is consistent: letter-value relationships do not surface in decontextualised lexical vectors; they do not surface in authorial passage construction; they surface in within-sentence attention geometry in both Arabic and Hebrew (at comparable magnitude on the Hebrew principal sample; attenuated but present on the like-for-like different-root subset, §4.6). The framework’s empirical case is therefore the *localisation* of the signal — not its overall magnitude. Letter-value relationships in classical Arabic operate as *relational, contextual, attention-resolvable structure*, not as static-vector properties or as authorial encoding choices in passage-level composition. This is consistent with the classical philological tradition’s own treatment of the Abjad system as an analytical instrument applied to inter-word relationships in context, rather than as a property of words in isolation.

### 5.2 What the cross-linguistic comparison establishes

The pre-registered Outcome A on the Hebrew AlephBERT control narrows the strongest reading of the Arabic attention-level result. The maximally strong reading — that the Mashriqi Abjad system specifically encodes structural information unavailable in adjacent letter-value systems — is not supportable: Hebrew gematria on AlephBERT produces an attention-geometry signal of comparable magnitude and direction on its principal all-pairs sample (with the confidently different-root subset attenuated to roughly half the Arabic magnitude; see the sample-definition note in §4.6). Whatever the attention signal is detecting, it is not Mashriqi-unique; it is a property of how classical-register Semitic templatic morphology surfaces in transformer geometry.
...
- **Hebrew root-extraction limit.** The Hebrew root classifier is heuristic and confidently resolves only a minority of word pairs into same-root or different-root categories: 1,468,657 confidently different-root and 58,776 same-root pairs out of 12,081,207 total (≈13%); 10,553,774 pairs are unresolved. All three categories are retained in the principal all-pairs sample, while the resolved subsets are reported separately. The Arabic root extraction had higher coverage, and the Arabic principal sample is a clean cross-root set. The cross-linguistic comparison should be read with caution regarding root-controlled subset analyses, though the primary Hebrew result on the all-pairs sample is well-resolved given the 12M sample.
